...
public class DiscordLinkCMD implements BotCommand {
    ...
    public void excecute(Command cmd, String[] args, Message msg, MessageReceivedEvent event) {
        ...
        Consumer<Message> success = (privateChannel) -> {

        };
        Consumer<Throwable> failure = (error) -> {
            EmbedBuilder embed = MessageUtils.getBuilder(Color.RED).setDescription(event.getAuthor().getAsMention() + ", je moet je privéberichten aanzetten!");
            event.getChannel().sendMessage(embed.build()).queue();
        ...
        event.getAuthor().openPrivateChannel().queue(privateChannel -> {
            ...
            privateChannel.sendMessage(embed.build()).queue(success, failure);
        });
    }
}